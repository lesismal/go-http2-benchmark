...
func main() {
	...
	defer be.Stop()
	beReport := be.Report()
	beReport.Pprof = report.PprofSetting(*framework, *echoPprof && pprofEnabled, *echoPprofDuration)
	saveReport(beReport)
	logging.Print(logging.ShortLine)
	...
	if *rateEnabled {
		...
		defer br.Stop()
		brReport := br.Report()
		brReport.Pprof = report.PprofSetting(*framework, *ratePprof && pprofEnabled, *ratePprofDuration)
		saveReport(brReport)
		logging.Print(logging.ShortLine)
