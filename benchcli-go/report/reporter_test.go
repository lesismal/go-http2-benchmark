...
func TestHiddenColumnsStayInTheJSON(t *testing.T) {
	...
	echo := &BenchEchoReport{Framework: "nethttp", BenchClient: "benchcli-go", CPUMin: 1, MEMRSSMin: 1}
	rate := &BenchRateReport{Framework: "gin", BenchClient: "benchcli-go", EchoEER: 12.5}
	conns := &ConnectionsReport{Framework: "nethttp", BenchClient: "benchcli-go"}
	...
	if !strings.Contains(JSON(rate), `"EchoEER":12.5`) {
		t.Errorf("BenchMultiplex JSON lost EchoEER: %s", JSON(rate))
	}
}
...
func TestSummaryTakesTheParametersOutOfTheTables(t *testing.T) {
	Init(false)
	conns := []Report{
		&ConnectionsReport{Framework: "fib", BenchClient: "benchcli-go", TPS: 9, Concurrency: 2000, MaxStreams: 250},
		&ConnectionsReport{Framework: "nethttp", BenchClient: "benchcli-go", TPS: 8, Concurrency: 2000, MaxStreams: 250},
		&ConnectionsReport{Framework: "gin", BenchClient: "benchcli-go", TPS: 7, Concurrency: 2000, MaxStreams: 250},
	}
	echo := []Report{
		&BenchEchoReport{Framework: "fib", BenchClient: "benchcli-go", Connections: 20000, Concurrency: 10000, Streams: 1, Total: 2000000, Payload: 1024},
		&BenchEchoReport{Framework: "nethttp", BenchClient: "benchcli-go", Connections: 19998, Concurrency: 10000, Streams: 1, Total: 2000000, Payload: 1024},
	}
	rate := []Report{
	...
	for _, table := range []string{Markdown(conns, false, SortResult, nil), Markdown(echo, false, SortResult, nil),
		...
		for _, column := range []string{"Client", "Conns", "Concurrency", "Payload", "Duration", "SendRate", "Batch", "Streams", "MaxStreams"} {
...
func TestMarkdownTableIsPerfsForASCII(t *testing.T) {
	for _, c := range []struct {
		...
		{[]string{"Framework", "TPS", "EER"}, [][]string{{"fib", "474843 100%", "1650.41"}, {"nethttp", "4", "1.5"}}},
		{[]string{"Parameter", "Value"}, [][]string{{"Conns", "20000 (fib); 19998 (nethttp, gin)"}, {"Payload", "1024"}}},
...
// TestRateTPSIsPacketsPerSecond puts BenchMultiplex's TPS right after
// Framework and its Lang,
...
// TestLangColumnFromFramework reads report files back, as the report step
// does, and gives each row its framework's language: the files do not carry
// it, whichever client wrote them.
func TestLangColumnFromFramework(t *testing.T) {
	Init(false)
	t.Chdir(t.TempDir())
	if err := os.MkdirAll("output/report", 0o755); err != nil {
		t.Fatal(err)
	}
	...
	if b, _ := os.ReadFile("output/report/h2-BenchEcho.json"); strings.Contains(string(b), "Lang") {
		t.Errorf("the JSON carries Lang: %s", b)
	}
	table := GenerateBenchEchoReports("", "", false, SortResult, nil)
