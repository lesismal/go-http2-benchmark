...
// connections at, exchanged SETTINGS on and answered each one's first request, which is what this
// benchmark measures. It has no EER to break a tie with: the connection test
// samples no CPU.
type ConnectionsReport struct {
	Framework string `json:"Framework" md:"Framework"`
	// Lang is the framework's language; see BenchEchoReport.Lang.
	...
	// MaxStreams is the SETTINGS_MAX_CONCURRENT_STREAMS the server sent,
	// the fewest of any connection's, which is what every benchmark's
	// streams per connection are held under.
	MaxStreams int `json:"MaxStreams" md:"MaxStreams" summary:"Max Streams"`
}
