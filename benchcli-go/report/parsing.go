...
// these headers come off them by reflection and ObjFieldValues reads the row
// the same way, so moving a field moves that column in every table. Framework
// comes first, since it is what a row is about, then the client that measured
// it.
//
...
// the connections, the payload, each benchmark's concurrency and so
