...
func TestServerProcessName(t *testing.T) {
	if got, want := ServerProcessName(NetHTTP), "nethttp.server"; got != want {
		t.Errorf("ServerProcessName(%q) = %q, want %q", NetHTTP, got, want)
	}
}
...
func TestVerifyServerProcess(t *testing.T) {
	if err := VerifyServerProcess(os.Getpid(), NetHTTP); err == nil {
		t.Errorf("VerifyServerProcess(%d, %q) = nil, want an error", os.Getpid(), NetHTTP)
	}
}
