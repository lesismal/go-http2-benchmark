...
Environment overrides:
  ...
  DOCKER_BENCH_IMAGE       Image tag (default: go-http2-benchmark:local)
  ...
  DOCKER_BENCH_RUST_IMAGE  Image the Rust toolchain is copied from
                           (default: rust:1-bookworm)
  DOCKER_BENCH_CARGO_MIRROR
                           Sparse crates.io index for the image's cargo fetch,
                           e.g. sparse+https://rsproxy.cn/index/
  (script/docker_benchmark_cn.sh sets the last five to mainland China mirrors.)

Examples:
  bash script/docker_benchmark.sh --smoke
  BENCH_FRAMEWORKS=fib,gin \
...
image=${DOCKER_BENCH_IMAGE:-go-http2-benchmark:local}
...
# benchmark.sh picks the servers' flags out of these itself.

timestamp=$(date +%Y%m%d-%H%M%S)
result_dir=${DOCKER_BENCH_OUTPUT:-"$repo_root/output/docker/$timestamp"}
mkdir -p "$result_dir"
container="go-http2-benchmark-${timestamp}-$$"
cleanup_container() {
...
Benchmark client: benchcli-$BENCH_CLIENT
